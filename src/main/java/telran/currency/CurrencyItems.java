...
public class CurrencyItems {
	...
	public static List<Item> getItems(CurrencyConvertor currencyConvertor) {
		...
		return List.of(Item.of("Strongest Currencies", io -> strongestWeakest(io, "Strongest")),
				Item.of("Weakest Currencies", io -> strongestWeakest(io, "Weakest")),
				Item.of("Convert currencies", CurrencyItems::convert), Item.ofExit());
	}

	private static void strongestWeakest(InputOutput io, String title) {
		int amount = io.readNumberRange(String.format("Enter amount of top %s currencies", title), "Wrong amount", 1,
	...
	private static void convert(InputOutput io) {
		String currencyFrom = io.readStringOptions("Enter currency from", "Wrong currency code", codes);
		String currencyTo = io.readStringOptions("Enter currency to", "Wrong currency code", codes);
